...
class Pager {
public:
    virtual int select_victim_frame(const vector<FTE>& frame_table) = 0;
...
public:
    ...
    int select_victim_frame(const vector<FTE>& frame_table) override {
        int frame_idx = hand;
...
public:
    ...
    int select_victim_frame(const vector<FTE>& frame_table) override {
        ...
        while (true) {
            const FTE& frame = frame_table[hand];
...
public:
    MMU(int num_frames, Pager* pager)
        ...
        for (int i = 0; i < num_frames; ++i) {
            ...
            free_frame_indices.push_back(i);
        }
    }
...
void MMU::loadInput(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        int value;
        iss >> operation >> value;
        instructions.emplace_back(operation, value);
    }
...
void MMU::handlePageFault(int vpage) {
    ...
    FTE& frame = frame_table[frame_idx];
...
void MMU::handleProcessExit(int procid) {
    ...
    for (int i = 0; i < MAX_VPAGES; ++i) {
        ...
        if (pte.present) {
            ...
            frame.pid = -1;
            frame.vpage = -1;
            free_frame_indices.push_back(pte.frame);
...
int MMU::get_frame() {
    if (!free_frame_indices.empty()) {
        int frame_idx = free_frame_indices.front();
        free_frame_indices.pop_front();
        return frame_idx;
    }
    int frame_idx = pager->select_victim_frame(frame_table);
    return frame_idx;
}
...
void MMU::printFrameTable() {
    if (F_option || f_option) {
        ...
        for (size_t i = 0; i < frame_table.size(); ++i) {
            const FTE& frame = frame_table[i];
            string output;

            if (frame.occupied) {
                output = to_string(frame.pid) + ":" + to_string(frame.vpage);
            } else {
                output = "*";
            }
...
int NRUPager::select_victim_frame(const vector<FTE>& frame_table) {
    size_t num_frames = frame_table.size();
    bool reset_referenced = false;
    int lowest_class = 4;
    int victim_idx = -1;
    int class_frame_indices[4] = {-1, -1, -1, -1};
    ...
    do {
        ...
        if (class_frame_indices[class_idx] == -1) {
            class_frame_indices[class_idx] = hand;
    ...
    victim_idx = class_frame_indices[lowest_class];
    hand = (victim_idx + 1) % num_frames;

    if (a_option) {
        cout << "ASELECT: " << start_hand << " " << (reset_referenced ? 1 : 0)
             << " | " << lowest_class << " " << victim_idx << "\n";
    }

    return victim_idx;
}

int AgingPager::select_victim_frame(const vector<FTE>& frame_table) {
    size_t num_frames = frame_table.size();
...
int WorkingSetPager::select_victim_frame(const vector<FTE>& frame_table) {
    size_t num_frames = frame_table.size();
    ...
    do {
        ...
        if (victim_idx == -1 || last_used[hand] < oldest_time) {
